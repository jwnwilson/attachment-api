...
def get_task_adapater(user_data: UserData = Depends(get_current_user)) -> TaskAdapter:
    queue_name = f"attachment-api_task_queue_{ENVIRONMENT}"
...
def get_db_adapater(user_data: UserData = Depends(get_current_user)) -> DbAdapter:
    table_name = f"attachment-api_task_{ENVIRONMENT}"
...
def get_task_storage_adapter(
    ...
    bucket_name = f"jwnwilson-attachment-api-task-{ENVIRONMENT}"
    return S3Adapter({"bucket": bucket_name}, user=user_data, upload_prefix="attachment-api-api")


def get_template_storage_adapter(
    user_data: UserData = Depends(get_current_user),
) -> StorageAdapter:
    bucket_name = f"jwnwilson-attachment-api-template-{ENVIRONMENT}"
    return S3Adapter({"bucket": bucket_name}, user=user_data, upload_prefix="attachment-api-api")
